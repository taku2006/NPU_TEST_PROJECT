...
class CLI(cmd.Cmd):
    ...
    def do_testtx(self, arg):
        ...
        k=1
        i = 1
        fp_ilk_in = open("new_in.txt", 'r')
        for line in fp_ilk_in:
            if len(line) != 129:
                continue
    ...
    def do_testrx(self, arg):
        ...
        for line in fp_ilk_in:
            ...
            dp = self.conver2hex(line)
            #print(dp)
            self.s.sendto(dp, (HOST, 0xfff1))
            data = self.s.recvfrom(1024)
            print('Port0 RX Line %d : Sent' %(k))
            k = k + 1
            time.sleep(0.005)
    ...
    def do_testtx2(self, arg):
        dp=''
        k=1
        fp_ilk_in = open("new_in_2.txt", 'r')
        for line in fp_ilk_in:
            if len(line) != 129:
                continue
            dp = self.conver2hex(line)
            #print(dp)
            self.s.sendto(dp, (HOST, 0xfff2))
            data = self.s.recvfrom(1024)
            print('Port0 TX Line %d : Sent' %(k))
            k = k + 1
            time.sleep(0.005)
    ...
    def do_testrx2(self, arg):
        dp=''
        k=1
        fp_ilk_in = open("new_out_2.txt", 'r')
        for line in fp_ilk_in:
            if len(line) != 129:
                continue
            #print(line)
